...
// ===================================
// FUNKCIONALNOST SWIPER MODALNOG PROZORA (Potpuno izdvojena i ispravna funkcija)
// ===================================

let modalSwiper = null; // Globalna varijabla
const imageModalElement = document.getElementById('imageModal'); // Dohvati modalni element

// 1. Definicija funkcije koja se poziva iz HTML-a (onclick="openModal(this)")
function openModal(element) {
    
    // 1. Pronalazimo element koji sadrži SVE sličice (swiper-category-wrapper)
    ...
    // 6. Otvori modal
    // Pretpostavlja da koristite Bootstrap 5
    const imageModal = new bootstrap.Modal(document.getElementById('imageModal'));
    imageModal.show();

    // 7. Inicijalizuj modal swiper NAKON ŠTO SE MODAL POTPUNO PRIKAŽE
    // Koristimo { once: true } da ne dupliramo event listenere
    document.getElementById('imageModal').addEventListener('shown.bs.modal', function () {
        
        // Uništi prethodnu instancu Swiper-a
        if (modalSwiper !== null) {
            modalSwiper.destroy(true, true);
        }
        ...
        modalSwiper = new Swiper('.modal-swiper', {
            loop: true,
            initialSlide: startIndex, // Počinje od kliknute slike
            navigation: {
                nextEl: '.custom-swiper-modal-next',
                prevEl: '.custom-swiper-modal-prev',
            },
...
// 2. Inicijalizacija Swipera kada je Modal POTPUNO OTVOREN (Ovo je bio duplikat)
// Ostavljamo ovaj event listener samo ako openModal nije definisan, što u Vašem slučaju jeste
// pa ga ne trebamo. Funkcija iznad će se pobrinuti za sve.